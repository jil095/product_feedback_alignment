...
class Linear(nn.Linear):
    ...
    def masked_weights(self):
        if self.b_conn_density < 1 or self.w_conn_density < 1:
            if not hasattr(self, "W_mask"):
                ...
                self.B_mask = torch.rand_like(self.weight_backward) < self.b_conn_density
            ...
            if self.W_mask.device != self.weight.device:
                ...
                self.B_mask = self.B_mask.to(self.weight_backward.device, dtype=self.weight_backward.dtype)
            with torch.no_grad():
                self.weight.data *= self.W_mask
                self.weight_backward.data *= self.B_mask
    ...
    def dfa_backward_hook(module, grad_input, grad_output):
        ...
        if grad_input[0] is None:
            ...
            grad_dfa = module.loss_gradient.mm(module.weight_backward.to(module.loss_gradient.dtype))
            # If no bias term
            if len(grad_input) == 2:
